 ...
 */
public class BezierPath extends ArrayList<BezierPath.Node> implements Shape {
  /**
   ...
   */
  public void set(int index, int coordinate, Point2D.Double p) {
    Node c = get(index);
    c.x[coordinate] = p.x;
    c.y[coordinate] = p.y;
  }
   ...
   */
  public Point2D.Double get(int index, int coordinate) {
    Node c = get(index);
    return new Point2D.Double(c.x[coordinate], c.y[coordinate]);
  }
  ...
  public boolean outlineContains(Point2D.Double p, double tolerance) {
    ...
    while (!i.isDone()) {
      ...
      if (Geom.lineContainsPoint(prevX, prevY, coordinates[0], coordinates[1], p.x, p.y, tolerance)) {
        return true;
      }
      prevX = coordinates[0];
      prevY = coordinates[1];
  ...
  public Point2D.Double getPointOnPath(double relative, double flatness) {
    ...
    for (; !i.isDone(); i.next()) {
      i.currentSegment(coordinates);
      len += Geom.length(prevX, prevY, coordinates[0], coordinates[1]);
      prevX = coordinates[0];
      prevY = coordinates[1];
    }
    ...
    type = i.currentSegment(coordinates);
    prevX = coordinates[0];
    prevY = coordinates[1];
    i.next();
    for (; !i.isDone(); i.next()) {
      i.currentSegment(coordinates);
      double segLen = Geom.length(prevX, prevY, coordinates[0], coordinates[1]);
      if (pos + segLen >= relativeLen) {
        //if (true) return new Point2D.Double(coordinates[0], coordinates[1]);
        // Compute the relative Point2D.Double on the line
                /*
                return new Point2D.Double(
                        prevX * pos / len + coordinates[0] * (pos + segLen) / len,
                        prevY * pos / len + coordinates[1] * (pos + segLen) / len
                        );*/
        double factor = (relativeLen - pos) / segLen;

        return new Point2D.Double(prevX * (1 - factor) + coordinates[0] * factor, prevY * (1 - factor) + coordinates[1] * factor);
      }
      pos += segLen;
      prevX = coordinates[0];
      prevY = coordinates[1];
  ...
  public int splitSegment(Point2D.Double split, float tolerance) {
    ...
    if (i != -1) {
      if ((get(i).mask & C2_MASK) == C2_MASK && (get(nextI).mask & C1_MASK) == 0) {
        ...
        // quadTo
        add(i + 1, new Node(C2_MASK, split, split, split));
      } else if ((get(i).mask & C2_MASK) == C2_MASK && (get(nextI).mask & C1_MASK) == C2_MASK) {
        // cubicTo
