 ...
 * the functionality needed to work with documents, such as a menu bar, toolbars and
 * palette windows.
 *
...
//    mo.putAction(MinimizeAction.ID, new MinimizeAction(this));
    ...
    if (!p.isShowing()) {
      ...
      p.addPropertyChangeListener(evt -> {
        ...
        if (name.equals("hasUnsavedChanges")) {
          ((JInternalFrame) f.getRootPane().getParent()).putClientProperty("windowModified", p.hasUnsavedChanges());
        } else if (name.equals("file")) {
          f.setTitle((p.getFile() == null) ? "Unnamed" : p.getFile().getName());
        }
      });

      f.addPropertyChangeListener(evt -> {
        String name = evt.getPropertyName();
  ...
  protected JMenuBar createMenuBar() {
    ...
    addPropertyChangeListener(evt -> {
      String name = evt.getPropertyName();
      ApplicationModel mo1 = getModel();
      if (Objects.equals(name, "projectCount")) {
        JMenu m1 = windowMenu;
        m1.removeAll();

        m1.add(mo1.getAction(ArrangeAction.CASCADE_ID));
        m1.add(mo1.getAction(ArrangeAction.VERTICAL_ID));
