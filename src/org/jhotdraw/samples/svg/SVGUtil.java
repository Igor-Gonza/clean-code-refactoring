...
//    map.put("aqua", new Color(0, 255, 255));
        ...
        if (type.equals("matrix")) {
          ...
          for (int i = 0; i < 6; i++) {
            ...
            if (tt.nextToken() == StreamTokenizer.TT_WORD && tt.sval.startsWith("E")) {
              ...
              tt.nval = Double.parseDouble(tt.nval + tt.sval);
            } else {
              tt.pushBack();
            }
            m[i] = tt.nval;
  ...
  private final static HashSet pathCommands = new HashSet(Arrays.asList("M", "m", "Z", "z", "L", "l", "H", "h", "V", "v", "C", "c", "S", "s", "Q", "q", "T", "t"));
  ...
  public static void readAttributes(Figure f, DOMInput in) throws IOException {
    ...
    if (value != null) {
      WINDING_RULE.set(f, value.equalsIgnoreCase("NONZERO") ? WindingRule.NON_ZERO : WindingRule.EVEN_ODD);
    } else {
    ...
    if (value != null) {
      ...
      for (int i = 0, n = dashes.length; i < n; i++) {
        dashes[i] = Double.parseDouble(tt.nextToken());
      }
      STROKE_DASHES.set(f, dashes);
    }
    value = getInheritedAttribute("stroke-dashoffset", in, styles);
    if (value != null) {
      STROKE_DASH_PHASE.set(f, Math.abs(Double.parseDouble(value)));
    }
  ...
  public static Map<String, String> getStyles(String str) throws IOException {
    ...
    while (tt.nextToken() != StreamTokenizer.TT_EOF) {
      if (tt.ttype != ';') {
        ...
        value = new StringBuilder(tt.sval);
        while (tt.nextToken() == StreamTokenizer.TT_WORD) {
          value.append(' ').append(tt.sval);
        }
        tt.pushBack();
