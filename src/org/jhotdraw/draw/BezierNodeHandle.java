...
public class BezierNodeHandle extends AbstractHandle {
  ...
  public void draw(Graphics2D g) {
    ...
    if (f.getPointCount() > index) {
      ...
      } else if (v.mask == BezierPath.C1_MASK || v.mask == BezierPath.C2_MASK || !f.isClosed() && v.mask == (BezierPath.C1_MASK | BezierPath.C2_MASK) && index == 0 || index == f.getNodeCount() - 1) {
  ...
  protected Point getLocation() {
    return getBezierFigure().getPointCount() > index ? view.drawingToView(getBezierFigure().getPoint(index, 0)) : new Point(10, 10);
  }
