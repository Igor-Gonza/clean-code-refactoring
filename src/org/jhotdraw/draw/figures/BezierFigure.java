...
public class BezierFigure extends AttributedFigure {
   ...
   */
  public Point2D.Double getPoint(int index, int coordinates) {
    return path.get(index).getControlPoint(coordinates);
  }

  /**
   * Sets the point coordinate of a control point.
   */
  public void setPoint(int index, int coordinates, Point2D.Double p) {
    willChange();
    basicSetPoint(index, coordinates, p);
    changed();
   ...
   * Sets the point coordinate of a control point.
   */
  public void basicSetPoint(int index, int coordinates, Point2D.Double p) {
    BezierPath.Node cp = new BezierPath.Node(path.get(index));
    cp.setControlPoint(coordinates, p);
