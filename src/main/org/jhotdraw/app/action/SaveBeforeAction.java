 ...
 * If the user chooses to discard the changes, toIt is invoked immediately.
...
public abstract class SaveBeforeAction extends AbstractProjectAction {
  ...
  public void actionPerformed(ActionEvent evt) {
    ...
    if (p.isEnabled()) {
      ...
      if (p.hasUnsavedChanges()) {
        ...
        JSheet.showSheet(pane, p.getComponent(), evt1 -> {
  ...
  protected void saveChanges(final Project p) {
    if (p.getFile() == null) {
      ...
      JSheet.showSaveSheet(fileChooser, p.getComponent(), evt -> {
